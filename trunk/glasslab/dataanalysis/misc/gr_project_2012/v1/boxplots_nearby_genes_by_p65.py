...
if __name__ == '__main__':
    ...
    for ratio in (1.5,2,3):    
        data = yzer.import_file(yzer.get_filename(dirpath, 'motifs', 'transcript_vectors_with_nearby_peaks.txt'))
        nearby = yzer.import_file(yzer.get_filename(img_dirpath, 
                            'nearest_genes_to_enhancer_like_{1}_p65_{0}x.txt'.format(str(ratio).replace('.','_'),
                                                                                     change_type)))
        ...
        if True:
            ...
            names = [s.format('p65') for s in ['No {0}','Loses {0}\nin KLA+Dex','No change in {0}', 'Gains {0}\nin KLA+Dex',
                                               'Near Enhancer\nthat has {1} {{0}} ({0}-fold)'.format(ratio, change_type)]]
            ...
            yzer.save_plot(yzer.get_filename(img_dirpath, 
                    '{2}_with_nearby_unique_{0}x_{3}_sampled_{1}.png'.format(ratio, random.randint(0,9999),
                                                                                colname, change_type)))
            yzer.show_plot()