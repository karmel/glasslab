...
if __name__ == '__main__':
    ...
    for rep in (4,3,1):
        ...
        ax = yzer.histogram(grouped['kla_{0}_lfc'.format(rep)], bins=50,
                         label='Replicate {0} Data'.format(rep),
                         show_legend=False, show_plot=False)
        
        ax = yzer.histogram(shuffled_grouped['kla_{0}_lfc'.format(rep)], bins=50,
        ...
        ax = yzer.histogram(grouped['dex_over_kla_{0}_lfc'.format(rep)], bins=50,
                         label='Replicate {0} Data'.format(rep),
                         show_legend=False, show_plot=False)
        
        ax = yzer.histogram(shuffled_grouped['dex_over_kla_{0}_lfc'.format(rep)], bins=50,
