...
def get_filters_many(subdata, xcol, ycol, xcol2, ycol2):
    none = subdata[xcol] + subdata[ycol] == 0
    ...
    if True:
        for main, compare, basal_cond, comp_cond in (('p65','GR', 'KLA', 'Dex'),('GR','p65', 'Dex', 'KLA')):
            ...
            totals = [len(data[filterset].groupby(['id','chr_name'],as_index=False).mean()) 
                                            for filterset in get_filters_transcript(data, tag_count_1, tag_count_2)
                                            if sum(filterset) > 0]
            ...
            for i, dataset in enumerate(datasets):
                subdatasets = [dataset[filterset].groupby(['id','chr_name'],as_index=False).mean()
                                            for filterset in get_filters_transcript(dataset,  tag_count_1, tag_count_2)]
            ...
            title = 'Change in {0} and {1} summed over whole RefSeq transcript'.format(main, compare)
            pyplot.title(title)
